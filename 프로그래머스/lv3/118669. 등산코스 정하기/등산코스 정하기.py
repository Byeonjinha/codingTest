...
def solution(n, paths, gates, summits):
    def get_min_intensity(): #가장 짧은 거리를 반환하는 함수
        currentPoint = []  # (intensity, 현재 위치)
        visited = [10000001] * (n + 1)

        # 모든 출발지를 우선순위 큐에 삽입
        for gate in gates:
            heappush(currentPoint, (0, gate))
            visited[gate] = 0

        # 산봉우리에 도착할 때까지 반복
        while currentPoint:
            intensity, node = heappop(currentPoint)
            ...
            for weight, next_node in graph[node]:
                ...
                if new_intensity < visited[next_node]:
                    ...
                    heappush(currentPoint, (new_intensity, next_node))
